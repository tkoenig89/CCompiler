...
fack:	# Beginning of a function. We will save the return adress $31 and the $fp.
	ADDI $sp, $sp, -8
	SW $31, 4($sp)
	SW $fp, 0($sp)
	MOVE $fp, $sp

	ADDI $sp, $sp, -24	#Allocate Memory on stackpointer for local Variables
	#int a: 20($sp)
	#int b: 16($sp)
	#int c: 12($sp)
	#int d: 8($sp)
...
l0:
	...
	MOVE $2, $5	#Return int
	ADDI $sp, $sp, 24	# delete local variables
	J l2
l1:
	LW $5, 8($fp)	#Parameter recognised:n
	LI $6, 1	#Number recognised:1
	...
	JAL fack	#Call function
	ADDI $sp, $sp, 4	# clean up stack after function call is done
	MOVE $16, $2	#Save return value by storing it into a temp register
	LW $5, 8($fp)	#Parameter recognised:n
	MUL $17, $5, $16	#Multiply 2 Variables and store result int temp register
	MOVE $2, $17	#Return .t3
	ADDI $sp, $sp, 24	# delete local variables
l2:
	#End of function fack. We will restore the return adress $31 and the $fp. Then we will jump back to where the func was called.
	LW $fp, 0($sp)
	LW $31, 4($sp)
	ADDI $sp, $sp, 8
	JR $31
